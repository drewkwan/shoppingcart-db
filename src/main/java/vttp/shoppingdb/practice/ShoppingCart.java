package vttp.shoppingdb.practice;

import java.io.Console;
import java.util.List;

//Shopping is what Kenneth calls the session. This is where we ahve our start function and all of our console commands

public class ShoppingCart {

    public static final String LIST = "list";
    public static final String ADD = "add";
    public static final String DEL = "del";
    public static final String CARTS = "carts";
    public static final String LOAD = "load";
    public static final String LOGIN = "login";
    public static final String SAVE = "save";
    public static final String USERS = "users";
    public static final String END = "end";

    private Repository repository;
    private ShoppingCartDB currCart;

    public void start() {

         Console cons = System.console();
         boolean stop = false;
         currCart = new ShoppingCartDB("anon");
         
         while (!stop) {
            String input = cons.readLine("> ");
            String[] terms = input.split(" ");
            String cmd = terms[0].toLowerCase();    
            switch(cmd) {
                case CARTS:
                    System.out.println("List of shopping carts: ");
                    //list carrrts db
                    break;
                
                case LIST:
                    System.out.printf("Items in %s's shopping cart\n", currCart.getUsername());
                    printList(currCart.getContents());
                    break;

                case ADD:
                    int before = currCart.getContents().size();
                    for(int i =1; i< terms.length; i++) {
                        currCart.add(terms[i]);
                    }
                    int addedCount = currCart.getContents().size() -before;
                    System.out.printf("Added %d item(s) to %s's cart\n", addedCount, currCart.getUsername());
                    break;

                case DEL:
                    int idx = Integer.parseInt(terms[1]);
                    String item = currCart.delete(idx -1);
                    System.out.println(item + "has been deleted from your cart!");
                    break;

                case LOAD:
                    currCart = repository.load(currCart.getUsername());
                    System.out.printf("Loaded %s's cart successfully! There are %s item(s).\n", currCart.getUsername(), currCart.getContents().size());
                    break;

                case SAVE:
                    repository.save(currCart);
                    System.out.println("List has been saved successfully!");
                    break;
                
                case USERS: 
                    List<String> allCarts = repository.getShoppingCarts();
                    this.printList(allCarts);
                    break;
                
                case END:
                    stop = true;
                    break;

                default:
                    System.out.printf("Invalid input %s!\n", terms[1]);
            }
            System.out.println("");
         }
        System.out.println("Thank you for shopping with us!");
    }

    public void printList(List<String> list) {
        if(list.size()<=0) {
            System.out.println("No record found");
            return;
        }

        for(int i = 0; i <list.size(); i++) {
            System.out.printf("%d. %s\n", (i+1), list.get(i));
        }
    }
   
    
}
